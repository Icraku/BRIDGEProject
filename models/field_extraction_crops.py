...
DEBUG = True  # <<< TURN OFF WHEN SATISFIED
...
def map_field(field, scale_x, scale_y):
...
def crop_field(image, coords):
    return image.crop(coords)


def extract_text(cropped):
...
def run():
    ...
    for idx, field in enumerate(FIELDS):
        ...
        coords = map_field(field, scale_x, scale_y)
        cropped = crop_field(image, coords)

        if DEBUG:
            crop_path = CROPS_DIR / f"{idx:02d}_{field['label'].replace(' ', '_')}.png"
            cropped.save(crop_path)

            draw.rectangle(coords, outline="red", width=3)
            draw.text((coords[0], coords[1] - 15), field["label"], fill="red")

        extracted = extract_text(cropped)
    ...
    if DEBUG:
        debug_image.save(OUTPUT_DIR / "debug_bounding_boxes.png")
